package roomescape.domain.payment.processor.toss;
...
import roomescape.infrastructure.payment.toss.TossPaymentProcessorErrorHandler;
import roomescape.infrastructure.payment.toss.exception.TossPaymentProcessException;
...
class TossPaymentProcessorIntegrationTest {
    ...
    void 에러응답_4xx를_받으면_커스텀_예외를_던진다() {
        ...
        assertThatThrownBy(() -> processor.processPayment(request))
                .isInstanceOf(TossPaymentProcessException.class)
    ...
    void 에러응답_400인데_Secret_KEY_에러를_받으면_커스텀_예외를_500상태로_던진다() {
        ...
        assertThatThrownBy(() -> processor.processPayment(request))
                .isInstanceOf(TossPaymentProcessException.class)
                .satisfies(exception -> {
                    TossPaymentProcessException tossPaymentException = (TossPaymentProcessException) exception;
