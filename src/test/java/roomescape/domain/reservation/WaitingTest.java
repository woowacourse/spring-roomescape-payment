...
class WaitingTest {
    @Test
    @DisplayName("예약 대기을 생성한다.")
    void create() {
        assertThatCode(() -> new Waiting(RESERVATION_DETAIL_1, MEMBER_1))
    ...
    void validateTime() {
        assertThatThrownBy(() -> new Waiting(null, MEMBER_1))
    ...
    void createWaiting() {
        ...
        Waiting reservation = Waiting.create(now, detail, MEMBER_1);
    ...
    @Test
    @DisplayName("create 메서드로 예약 대기을 생성할 때 지나간 날짜/시간이면 예외가 발생한다.")
    void createWaitingWhenPastDateTime() {
        LocalDate reservationTime = LocalDate.of(2024, 5, 5);
        LocalDateTime currentDateTime = LocalDate.of(2024, 5, 6).atTime(10, 0);

        ReservationDetail detail = new ReservationDetail(reservationTime, RESERVATION_TIME_1, THEME_1);

        assertThatThrownBy(() -> Waiting.create(currentDateTime, detail, MEMBER_1))
                .isInstanceOf(DomainValidationException.class)
                .hasMessage(String.format("지나간 날짜/시간에 대한 예약 대기는 불가능합니다. (예약 날짜: %s, 예약 시간: %s)",
