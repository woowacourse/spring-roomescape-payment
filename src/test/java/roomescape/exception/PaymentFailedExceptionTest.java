...
class PaymentFailedExceptionTest {
    ...
    void causedByServer() {
        var paymentFailure = new PaymentFailure(PaymentFailCode.INVALID_AUTH_CREDENTIALS, "키 인증 실패");
        var exception = new PaymentFailedException(paymentFailure);
    ...
    void causedByExternalServer() {
        var paymentFailure = new PaymentFailure(PaymentFailCode.EXTERNAL_SERVER_PROCESSING, "외부 서버 결제 실패");
        var exception = new PaymentFailedException(paymentFailure);
