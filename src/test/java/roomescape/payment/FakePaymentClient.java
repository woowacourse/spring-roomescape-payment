...
public class FakePaymentClient implements PaymentClient {
    ...
    @Override
    public PaymentInfoFromClient confirm(final ConfirmPaymentRequest confirmPaymentRequest) {
        if (confirmPaymentRequest.paymentKey().equals(FAIL_KEYWORD)) {
            throw new ClientException("결제 오류입니다. 같은 문제가 반복된다면 문의해주세요.");
        }

        return new PaymentInfoFromClient(confirmPaymentRequest.paymentKey(), confirmPaymentRequest.orderId(),
