...
class ThemeControllerTest extends BaseControllerTest {
    ...
    Stream<DynamicTest> failWhenNotFoundId() {
        return Stream.of(
                DynamicTest.dynamicTest("어드민이 로그인한다.", this::adminLogin),
                DynamicTest.dynamicTest("존재하지 않는 테마를 삭제한다.", this::deleteThemeByIdFailWhenNotFoundId)
        );
    }

    @TestFactory
    @DisplayName("이미 사용 중인 테마를 삭제하면 실패한다.")
    Stream<DynamicTest> failWhenUsedTheme() {
        return Stream.of(
                DynamicTest.dynamicTest("어드민이 로그인한다.", this::adminLogin),
                DynamicTest.dynamicTest("이미 사용 중인 테마를 삭제한다.", this::deleteThemeByIdFailWhenUsedTheme)
        );
    }
