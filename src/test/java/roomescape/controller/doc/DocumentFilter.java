...
public enum DocumentFilter {
    ...
    GET_THEMES("getThemes",
            responseFields(
                    ...
                    fieldWithPath("[].price").description("방탈출 테마 가격")
            )
    ),
    SAVE_RESERVATION_WAITING("saveReservationWaiting",
            requestCookies(cookieWithName("token").description("예약 대기를 생성하는 사용자의 토큰")),
            requestFields(
                    fieldWithPath("date").description("방탈출 대기 날짜"),
                    fieldWithPath("timeId").description("방탈출 대기 시간 ID"),
                    fieldWithPath("themeId").description("방탈출 대기 테마 ID")
            )
    );
