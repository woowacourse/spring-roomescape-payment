...
@WebMvcTest(controllers = ReservationApiController.class)
class ReservationApiControllerTest extends AbstractRestDocsTests {
    ...
    void 예약을_생성할_수_있다() throws Exception {
        ReservePaymentRequest request = new ReservePaymentRequest(
                LocalDate.of(2024, 6, 1), 1L, 1L,
                new PaymentInfoRequest("payKey", "orderId", 10000L, "NORMAL")
        );
        ...
        given(reservationService.pending(any(), any())).willReturn(response);
        ...
        mockMvc.perform(post("/reservations")
                ...
                .andDo(restDocs.document(
                        requestFields(
                                ...
                                fieldWithPath("payment.orderId").description("주문 번호"),
                                fieldWithPath("payment.amount").description("결제 금액"),
                                fieldWithPath("payment.paymentType").description("결제 타입")
                        ),
                        responseFields(
    ...
    void 지난_날짜_예약시_예외가_발생한다() throws Exception {
        ReservePaymentRequest request = new ReservePaymentRequest(
                LocalDate.of(2020, 1, 1), 1L, 1L,
                new PaymentInfoRequest("payKey", "orderId", 10000L, "NORMAL")
        );
        given(reservationService.pending(any(), any())).willThrow(new PastReservationException());
    ...
    void 이미_예약된_시간이면_예외가_발생한다() throws Exception {
        ...
        given(reservationService.pending(any(), any())).willThrow(new InAlreadyReservationException("이미 예약된 시간입니다."));
    ...
    @Test
    void 내_예약_목록을_조회할_수_있다() throws Exception {
        MyReservationResponse myReservation = new MyReservationResponse(
                1L, "공포", LocalDate.of(2024, 6, 1), LocalTime.of(10, 0), "RESERVED", null
        );
        given(reservationService.getAllReservations(any())).willReturn(List.of(myReservation));
        ...
        mockMvc.perform(get("/reservations/me"))
                ...
                .andDo(restDocs.document(
                        responseFields(
                                ...
                                fieldWithPath("message").description("응답 메시지"),
                                fieldWithPath("data[].id").description("예약 ID"),
                                fieldWithPath("data[].theme").description("테마 이름"),
                                fieldWithPath("data[].date").description("예약 날짜"),
                                fieldWithPath("data[].time").description("예약 시간"),
                                fieldWithPath("data[].status").description("예약 상태"),
                                fieldWithPath("data[].order").description("결제 정보").optional()
                        )
                ));
    }
    ...
    @Test
    void 본인_예약이_아니라면_예외가_발생한다() throws Exception {
        doThrow(new NotAuthorizationException("해당 예약자가 아닙니다."))
                .when(reservedService).cancel(any(), any());
    ...
    @Test
    void 존재하지_않는_예약_취소시_예외가_발생한다() throws Exception {
