...
function onReservationButtonClick(event, paymentWidget) {
    ...
    if (selectedDate && selectedThemeId && selectedTimeId) {
        ...
        */
        ...
        function postToPaymentsEndpoint(orderId, amount) {
            return fetch("/payments", {
                ...
                .then(response => {
                    if (!response.ok) {
                        return response.json().then(errorBody => {
                            console.error("결제 요청 실패 : " + JSON.stringify(errorBody));
                ...
                .then(paymentData => {
                    console.log("결제 요청 성공 : " + JSON.stringify(paymentData));
                    return paymentData;
                })
                .catch(error => {
                    console.error("결제 요청 에러: ", error.message);
