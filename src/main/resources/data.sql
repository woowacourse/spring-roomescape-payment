...
INSERT INTO reservations(reservation_slot_id, member_id, created_at, reservation_status)
VALUES
    -- ReservationSlot ID 1 (논리, 5/10, 08:00) - Alice(현재), Bob(대기), Carol(대기)
    (1, 3, '2025-05-09 10:00:00', 'CONFIRMED'),   -- Alice
    (1, 4, '2025-05-09 10:30:00', 'REQUESTED'),   -- Bob
    (1, 5, '2025-05-09 11:00:00', 'REQUESTED'),   -- Carol

    -- ReservationSlot ID 2 (논리, 5/10, 12:00) - Dave(현재), Eve(대기)
    (2, 6, '2025-05-09 11:30:00', 'CONFIRMED'),   -- Dave
    ...
    (4, 10, '2025-05-09 13:30:00', 'REQUESTED'),  -- Heidi

    -- ReservationSlot ID 5 (논리, 5/10, 18:00) - Ivan(현재), Judy(대기), Mallory(대기)
    (5, 11, '2025-05-09 14:00:00', 'CONFIRMED'),  -- Ivan
    (5, 12, '2025-05-09 14:30:00', 'REQUESTED'),  -- Judy
    (5, 13, '2025-05-09 15:00:00', 'REQUESTED'),  -- Mallory

    -- ReservationSlot ID 6 (로맨스, 5/10, 08:00) - Niaj(현재), Olivia(대기)
    (6, 14, '2025-05-09 15:30:00', 'CONFIRMED'),  -- Niaj
    (6, 15, '2025-05-09 16:00:00', 'REQUESTED'),  -- Olivia

    -- ReservationSlot ID 7 (로맨스, 5/10, 12:00) - Peggy(현재)
    (7, 16, '2025-05-09 16:30:00', 'CONFIRMED'),  -- Peggy

    -- ReservationSlot ID 8 (로맨스, 5/10, 14:00) - Rupert(현재), Sybil(대기)
    (8, 17, '2025-05-09 17:00:00', 'CONFIRMED'),  -- Rupert
    (8, 18, '2025-05-09 17:30:00', 'REQUESTED'),  -- Sybil

    -- ReservationSlot ID 9 (로맨스, 5/10, 16:00) - Trent(현재), Uma(대기)
    (9, 19, '2025-05-09 18:00:00', 'CONFIRMED'),  -- Trent
    (9, 20, '2025-05-09 18:30:00', 'REQUESTED'),  -- Uma

    -- ReservationSlot ID 10 (로맨스, 5/10, 18:00) - Victor(현재)
    (10, 21, '2025-05-09 19:00:00', 'CONFIRMED'), -- Victor

    -- ReservationSlot ID 11 (모험, 5/10, 08:00) - Wendy(현재), Xander(대기)
    (11, 22, '2025-05-09 19:30:00', 'CONFIRMED'), -- Wendy
    (11, 23, '2025-05-09 20:00:00', 'REQUESTED'), -- Xander

    -- ReservationSlot ID 12 (모험, 5/10, 12:00) - Yvonne(현재), Zack(대기)
    (12, 24, '2025-05-09 20:30:00', 'CONFIRMED'), -- Yvonne
    (12, 25, '2025-05-09 21:00:00', 'REQUESTED'), -- Zack

    -- ReservationSlot ID 13 (모험, 5/10, 14:00) - Amy(현재)
    (13, 26, '2025-05-09 21:30:00', 'CONFIRMED'), -- Amy

    -- ReservationSlot ID 14 (모험, 5/10, 16:00) - Brian(현재), Chloe(대기)
    (14, 27, '2025-05-09 22:00:00', 'CONFIRMED'), -- Brian
    (14, 28, '2025-05-09 22:30:00', 'REQUESTED'), -- Chloe

    -- 5월 11일 예약들
    -- ReservationSlot ID 15 (SF, 5/11, 08:00) - Daniel(현재), Alice(대기)
    (15, 29, '2025-05-10 10:00:00', 'CONFIRMED'), -- Daniel
    (15, 3, '2025-05-10 10:30:00', 'REQUESTED'),  -- Alice (다른 예약)

    -- ReservationSlot ID 16 (SF, 5/11, 12:00) - Bob(현재), Carol(대기)
    (16, 4, '2025-05-10 11:00:00', 'CONFIRMED'),  -- Bob
    (16, 5, '2025-05-10 11:30:00', 'REQUESTED'),  -- Carol

    -- ReservationSlot ID 17 (SF, 5/11, 14:00) - Dave(현재), Eve(대기), Frank(대기)
    (17, 6, '2025-05-10 12:00:00', 'CONFIRMED'),  -- Dave
    (17, 7, '2025-05-10 12:30:00', 'REQUESTED'),  -- Eve
    (17, 8, '2025-05-10 13:00:00', 'REQUESTED'),  -- Frank

    -- ReservationSlot ID 18 (감성, 5/11, 08:00) - Grace(현재)
    (18, 9, '2025-05-10 13:30:00', 'CONFIRMED'),  -- Grace

    -- ReservationSlot ID 19 (감성, 5/11, 12:00) - Heidi(현재), Ivan(대기)
    (19, 10, '2025-05-10 14:00:00', 'CONFIRMED'), -- Heidi
    (19, 11, '2025-05-10 14:30:00', 'REQUESTED'); -- Ivan
