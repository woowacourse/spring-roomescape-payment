...
public abstract class ReservationTimeControllerApi {
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "400", content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(
                                    name = "이미 존재하는 시간대",
                                    description = "이미 서버에 존재하는 시간대를 생성하려는 경우.",
                                    value = "detail: 생성하려는 시간 11:00가 이미 존재합니다. 시간을 생성할 수 없습니다.")))},
            parameters = @Parameter(name = "createReservationTimeRequest", description = "생성하려는 시간 정보를 담은 객체", required = true))
    abstract ResponseEntity<CreateReservationTimeResponse> createReservationTime(CreateReservationTimeRequest createReservationTimeRequest);
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "404", content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(
                                    name = "존재하지 않는 시간대",
                                    description = "서버에 존재하지 않는 시간대를 조회하려는 경우.",
    ...
    @Operation(
            summary = "방탈출 시간대 삭제",
            description = "방탈출 시간대를 삭제합니다.",
            responses = {
                    @ApiResponse(responseCode = "204", description = "시간대 삭제 성공"),
                    @ApiResponse(responseCode = "400", content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(
                                    name = "사용 중인 시간대",
                                    description = "해당 시간대에 대한 예약이 존재하는 경우",
                                    value = "detail: 식별자 1인 시간을 사용 중인 예약이 존재합니다. 삭제가 불가능합니다."))),
                    @ApiResponse(responseCode = "404", content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(
                                    name = "존재하지 않는 시간대",
                                    description = "거절하려는 시간대의 식별자 대한 정보가 서버에 존재하지 않는 경우",
                                    value = "detail: 식별자 1에 해당하는 시간이 존재하지 않습니다. 삭제가 불가능합니다.")))},
