...
public abstract class AdminWaitingControllerApi {
    ...
    @Operation(summary = "전체 대기 목록 조회", description = "모든 회원의 대기 목록을 조회합니다.", responses = {
            @ApiResponse(responseCode = "200", description = "전체 대기 목록 조회 성공", content = @Content(schema = @Schema(implementation = FindWaitingResponse.class)))
    })
    abstract ResponseEntity<List<FindWaitingResponse>> getWaitings();

    @Operation(summary = "방탈출 대기 거절", description = "회원의 대기를 거절합니다.", responses = {
            @ApiResponse(responseCode = "204", description = "예약 대기 거절 성공"),
            @ApiResponse(responseCode = "403", content = @Content(examples = {
                    @ExampleObject(name = "현재 로그인된 유저의 권한으로는 대기를 거절할 수 없는 경우", value = "회원의 권한이 없어, 식별자 1인 예약 대기를 삭제할 수 없습니다.")
            })),
