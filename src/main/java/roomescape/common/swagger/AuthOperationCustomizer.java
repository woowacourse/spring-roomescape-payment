...
public class AuthOperationCustomizer implements OperationCustomizer {
    ...
    private boolean hasSecurityRequirement(HandlerMethod handlerMethod) {
        return handlerMethod.getMethod().isAnnotationPresent(SecurityRequirement.class)
    ...
    private ApiResponses generateAuthErrorResponse(Operation operation) {
        ...
        Example noTokenCookieExample = new Example()
                .externalValue("token 쿠키 없음")
                .description("쿠키는 있지만 token에 대한 담은 쿠키가 없는 경우")
                .value(getExampleJsonString(ProblemDetail.forStatusAndDetail(HttpStatus.UNAUTHORIZED, "쿠키가 없어서 회원 정보를 찾을 수 없습니다. 다시 로그인해주세요.")));
        ...
        mediaType.addExamples("token 쿠키 없음", noTokenCookieExample);
    ...
    private void hideAuthInfoParameter(Operation operation) {
        if (operation.getParameters() != null) {
            operation.getParameters().removeIf(parameter -> parameter.getName().equals("authInfo"));
        }
    }
}
