...
public class ReservationTimeService {
    ...
    public ReservationTimeResponse save(ReservationTimeRequest reservationTimeRequest) {
        ...
        if (existsByStartAt(startAt)) {
            throw new RoomescapeException(String.format("중복된 예약 시간입니다. 요청 예약 시간:%s", startAt));
        }
    ...
    public void deleteById(Long id) {
        ...
        if (reservedCount > 0) {
            throw new RoomescapeException(String.format("해당 예약 시간에 연관된 예약이 존재하여 삭제할 수 없습니다. 삭제 요청한 시간:%s", findReservationTime.getStartAt()));
        }
