...
public interface GlobalExceptionHandlerApi {
    ...
    ResponseEntity<ErrorResponse> handleClientPaymentException(PaymentException e);

    ResponseEntity<ErrorResponse> handleUnauthorizedException(UnauthorizedException e);

    ResponseEntity<ErrorResponse> handleForbiddenException(ForbiddenException e);

    ResponseEntity<List<ErrorResponse>> handleValidationException(MethodArgumentNotValidException e);

    ResponseEntity<ErrorResponse> handleIllegalArgumentException(IllegalArgumentException e);

    @ApiResponse(responseCode = "500", description = "비즈니스 로직 외 서버 오류", content = @Content(
            mediaType = "application/json",
            examples = @ExampleObject(name = "비즈니스 로직 외 서버 오류",
