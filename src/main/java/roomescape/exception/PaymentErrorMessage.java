...
public enum PaymentErrorMessage {
    ...
    EXCEED_MAX_CARD_INSTALLMENT_PLAN(400, "최대 할부 개월 수를 초과했습니다."),
    ...
    PaymentErrorMessage(int statusCode, String errorMessageKorean) {
        this.statusCode = HttpStatusCode.valueOf(statusCode);
        this.errorMessageKorean = errorMessageKorean;
    }

    public static PaymentErrorMessage from(String name) {
        return Arrays.stream(values())
                .filter(errorMessage -> errorMessage.name().equalsIgnoreCase(name))
                .findFirst()
                .orElse(DEFAULT);
    }
    ...
    public String getErrorMessageKorean() {
        return errorMessageKorean;
    }
}
