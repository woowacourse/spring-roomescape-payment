...
public class ReservationService {
    ...
    public ReservationResponse saveReservation(final ReservationRequest request, final LoginMember loginMember) {
        ...
        if (reservationRepository.existsByDateAndTimeAndTheme(request.date(), reservationTime, theme)) {
            return new ReservationResponse(
                    waitingReservation(request.date(), reservationTime, theme, loginMember));
        }
        ...
        return new ReservationResponse(
                bookedReservation(request.date(), reservationTime, theme, loginMember,
                        request.orderId(), request.paymentKey(), request.amount())
        );
    }

    private Reservation waitingReservation(
            LocalDate date, ReservationTime reservationTime, Theme theme, LoginMember loginMember
    ) {
        ...
        Reservation reservation = Reservation.waiting(
                date, reservationTime, theme, member, LocalDateTime.now(clock), lastWaitingRank + 1
        );
    ...
    private Reservation bookedReservation(
        ...
        Payment payment = paymentRepository.save(Payment.from(orderId, paymentKey, amount));

        Reservation reservation = Reservation.of(
                date, reservationTime, theme, member,
                LocalDateTime.now(clock), payment
        );
        return reservationRepository.save(reservation);
    }
    ...
    public ReservationResponse saveAdminReservation(final AdminReservationRequest request) {
        ...
        final Reservation reservation = Reservation.admin(
                request.date(), reservationTime, theme, member, LocalDateTime.now(clock)
        );
