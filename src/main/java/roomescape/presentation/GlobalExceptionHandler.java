...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
        ...
        log.warn("MethodArgumentNotValid error: {}", fieldErrors);
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(
        ...
        log.warn("HttpMessageNotReadable error: {}", problemDetail.getProperties().get("message"));
        return ResponseEntity.badRequest().body(problemDetail);
    }
    ...
    public ProblemDetail handleInvalidInput(final InvalidInputException ex) {
        log.warn("InvalidInput error: {}", ex.getMessage());
        return createProblemDetail(BAD_REQUEST, "올바르지 않은 입력값입니다.", ex.getMessage());
    }
    ...
    public ProblemDetail handleInUse(final InUseException ex) {
        log.warn("InUse error: {}", ex.getMessage());
    ...
    public ProblemDetail handleAlreadyExisted(final AlreadyExistedException ex) {
        log.warn("AlreadyExisted error: {}", ex.getMessage());
    ...
    public ProblemDetail handleBusinessRuleViolation(final BusinessRuleViolationException ex) {
        log.warn("BusinessRuleViolation error: {}", ex.getMessage());
    ...
    public ProblemDetail handleAuthorization(final AuthorizationException ex) {
        ...
        return createProblemDetail(FORBIDDEN, "인가에 실패했습니다.", "접근 권한이 없습니다.");
    }
    ...
    public ProblemDetail handlePaymentException(final ExternalApiException ex) {
        if (ex.getErrorCode().getHttpStatus() == INTERNAL_SERVER_ERROR) {
            log.error("ExternalApiException error: {}", ex.getMessage(), ex.getCause());
            return createProblemDetail(
                    ex.getErrorCode().getHttpStatus(),
                    "외부 시스템과의 연동 중 문제가 발생했습니다.",
                    "내부 서버 오류가 발생했습니다."
            );
        }
        log.error("ExternalApiException error: {}", ex.getMessage());
        return createProblemDetail(ex.getErrorCode().getHttpStatus(), "외부 시스템과의 연동 중 문제가 발생했습니다.", ex.getMessage());
    }

    @ExceptionHandler(Exception.class)
    @ResponseStatus(code = INTERNAL_SERVER_ERROR)
    public ProblemDetail handleException(final Exception ex) {
        log.error("Exception error: {}", ex.getMessage(), ex.getCause());
