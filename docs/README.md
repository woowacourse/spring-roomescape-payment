...
- [x] 결제 승인 API 호출 실패 시, 예외 발생
  - [x] 클라이언트에게 결제 실패 사유 안내
- [x] 내 예약 페이지에서 예약 정보 외 결제 정보도 함께 볼 수 있도록 수정
  - [x] 필수 결제 정보 : paymentKey, 결제 금액

### 배포 및 로깅
- [x] 클라우드 서버에 방탈출 예약 서비스 배포
  - 자세한 배포 과정은 [배포 가이드](docs/deploy.md) 참고
- [x] 사용자 예약 페이지에서 호출하는 모든 API를 확인할 수 있는 문서 작성
- [x] 로깅 처리 추가
