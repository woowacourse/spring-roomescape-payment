## ERD
![스크린샷 2025-06-05 오후 5.38.17.png](erd.png)
...
- [x] "/reservations" get 요청 시 모든 예약 정보를 반환한다.
    - [x] 검색 조건(name, date, themeId)을 쿼리 파라미터로 전달하여 예약을 검색할 수 있다.
...
### 결제
- [x] "/reservations" POST 요청을 통해 예약과 함께 결제를 요청할 수 있다.
  - [x] 결제 정보(paymentKey, orderId, amount)를 포함하여 요청한다.
  - [x] 결제 API 호출이 실패한 경우 PaymentRequestException이 발생한다.
- [x] "/reservations/me" GET 요청을 통해 사용자의 예약과 결제 정보를 조회할 수 있다.
  - [x] 결제 정보를 찾을 수 없는 경우 "결제 정보를 찾을 수 없습니다." 메시지가 반환된다.

### 예약 대기
- [x] "/reservations/waitings" post 요청으로 사용자 예약 대기를 할 수 있다.
    - [x] 응답에 예약 대기 순위 정보가 포함된다.
