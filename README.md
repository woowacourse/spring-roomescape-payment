...
- 최근 일주일을 기준으로 하여 해당 기간 내에 방문하는 예약이 많은 테마 5개를 보여준다.
...
- 사용자는 예약 가능한 시간을 확인하고, 원하는 시간에 예약을 할 수 있다.
    - 시간을 선택하고 결제를 하면 예약이 완료된다.


- 이미 예약이 완료된 시간이라면, 예약 대기가 가능하다.
    - 과거의 날짜는 대기할 수 없다.
    - 예약 대기는 예약이 완료된 시간에만 가능하다.
    - 예약 대기는 중복으로 불가능하다.
    - 같은 시간에 예약이 존재하는 경우 대기는 불가능하다.
- 예약 대기 시, 예약이 취소되면 자동으로 예약이 완료된다.


- 사용자는 본인의 예약을 확인할 수 있다.
    - 본인 예약 조회 시, 예약 날짜, 예약 시간, 방탈출 테마, 예약 여부가 표기 된다.
    - 예약 여부는 예약과 대기로 나뉘며, 몇 번째 대기인지도 표기된다.
...
- 어드민 시간 관리 페이지에서만 각각 추가, 삭제 기능을 사용할 수 있다.
- 어드민 테마 관리 페이지에서만 각각 추가, 삭제 기능을 사용할 수 있다.
    - 테마 이름은 10자 이하여야 한다.
    - 테마 설명은 50자 이하여야 한다.
- 어드민 대기 관리 페이지에서 예약 대기를 관리할 수 있다.
    - 예약 대기 조회가 가능하다.
...
- 예외 처리 시, CustomException 클래스를 작성하여 사용한다.
    ...
    - PaymentException(PaymentErrorCode): 토스 결제 예외
